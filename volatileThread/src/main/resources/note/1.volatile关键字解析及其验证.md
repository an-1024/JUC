...
* [Volatile 关键字](#volatile-关键字)
   * [1.可见性：](#1可见性)
   * [2.不保证原子性](#2不保证原子性)
      * [解决 volatile 不保证原子性](#解决-volatile-不保证原子性)
   * [3.禁止指令重排序(有序性)](#3禁止指令重排序有序性)
      * [三种重排序的场景](#三种重排序的场景)
      * [指令重排的验证](#指令重排的验证)
   * [volatile 性能如何](#volatile-性能如何)
      * [CPU封装架构](#cpu封装架构)
      * [缓存行和伪共享](#缓存行和伪共享)
      * [缓存一致性协议](#缓存一致性协议)
         * [已修改Modified (M)](#已修改modified-m)
         * [独占Exclusive (E)](#独占exclusive-e)
         * [共享Shared (S)](#共享shared-s)
         * [无效Invalid (I)](#无效invalid-i)
...
```text
0x0000000117ac040b: shl    $0x3,%rsi          ;*getstatic atomicInteger
                                            ; - com.anzhi.atomicity.AtomicityDemo::increase@0 (line 14)

0x0000000117ac040f: cmp    (%rsi),%rax        ;*invokevirtual getAndIncrement
                                            ; - com.anzhi.atomicity.AtomicityDemo::increase@3 (line 14)
                                            ; implicit exception: dispatches to 0x0000000117ac0501
0x0000000117ac0412: mov    %rsi,%rdi
```

可以看到汇编也是分三步进行, 那为什么还能保证并发的正确性呢？看一下 AtomicInteger 类的 getAndIncrement 实现:
...
在 AtomicInteger 实例化的时候 首先执行 静态类初始化。使用 `unsafe.objectFieldOffset (AtomicInteger.class.getDeclaredField("value"))`
这段代码获取 `value` 值的内存地址, 而变量 `value` 又被 `volatile` 修饰保证可见性。

getAndIncrement 方法调用饿了 Unsafe 类中的 getAndAddInt 方法:
...
```
在这个循环中调用了两个本地方法, getIntVolatile 从内存中获取真实的 `value` 值,  `compareAndSwapInt` 则比较**当前对象值**和 var5 比较, 不同则
则进入循环继续比较, 反之则更新退出循环。 这里需要注意的是每次只能有一个线程执行 CAS 操作。 CAS 是什么? CAS 是一条 CPU 的并发原语, 它的功能是判
断内存某个位置的值是否为预期值，如果是则更改为新的值，这个过程是原子的。

通过上述对 getAndAddInt 解读, 需要注意的是 AtomicInterger 只能对一个整形变量进行原子操作, 如果是多个就不行了。

来看下 CAS 在 JVM 源码中的实现:compareAndSwapInt 的源码在 jdk8u-dev/hotspot/src/share/vm/prims/unsafe.app 路径下，它的源码实现是
...
这段代码的核心在与 `Atomic::cmpxchg` cmpxchg 是 x86 CPU 架构的汇编指令, 它的主要作用就是比较并交换操作数。
...
观察结果和手动 padding 效果差不多。

总结一下 volatile 的优化方案：其实也比较简单, 就是充分利用缓存行填充原理, 尽量使两个 volatile 变量不在一个缓存行中。
