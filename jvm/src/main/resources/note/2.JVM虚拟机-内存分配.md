...
看下测试结果：
```text
GCObject 将要被回收，id = 522305 name=9f8ae6a9-a87a-4ab2-8643-7f327b86a621
GCObject 将要被回收，id = 522306 name=6afc436f-3a36-4170-9090-f482f2157359
GCObject 将要被回收，id = 522307 name=3cc7df23-ac2e-41c5-add2-2baaff07f133
```
为什么加了引用还是被回收了？这是因为每个类的 finalize 只会被执行一次，就不会再执行了。所以对象还是被回收了。
