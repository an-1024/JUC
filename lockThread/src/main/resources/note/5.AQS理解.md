...
前面在`acquireQueued`方法中挂起线程的时候，如果出现异常，在 `finally`中会根据 `failed`的值进行判断是否要取消`cancelAcquire(node)`入队操作。来看下是如何处理的：
...
private void cancelAcquire(Node node) {
  ...
  // CAS 操作，将当前节点脱离AQS队列
  // 注意，Node 节点是尾节点，将当前节点替换为上一个节点
  if (node == tail && compareAndSetTail(node, pred)) {
    compareAndSetNext(pred, predNext, null);
  } else {

    // 上面CAS操作失败或者node不是尾节点
    int ws;
    // 不是 head 的后继节点
    if (pred != head &&
        // 获取到上一个节点的状态是否为-1，
        ((ws = pred.waitStatus) == Node.SIGNAL ||
         // 如果不是-1但小于等于0，则将该节点的waitStatus状态设置为-1
         (ws <= 0 && compareAndSetWaitStatus(pred, ws, Node.SIGNAL))) &&
        pred.thread != null) {
      Node next = node.next;
      // 为了确保后继节点可以被唤醒，所以上面才需要将 waitStatus 设置为1
      if (next != null && next.waitStatus <= 0)
        // 将上一个节点的next指针断开，指向取消节点的下一个节点
