...
public class HasSelfPrivateNum {
    ...
    synchronized public void addMethod(HasSelfPrivateNum hasSelfPrivateNum, String userName) {
        System.out.println("a 线程获得锁, 对象头信息为: " + ClassLayout.parseInstance(hasSelfPrivateNum).toPrintable());
        num += 100;
...
```text
b 线程获得锁, 对象头信息为: com.anzhi.twoobjecttwolock.HasSelfPrivateNum object internals:
...
a 线程获得锁, 对象头信息为: com.anzhi.twoobjecttwolock.HasSelfPrivateNum object internals:
 OFFSET  SIZE   TYPE DESCRIPTION                               VALUE
      0     4        (object header)                           60 89 be 11 (01100000 10001001 10111110 00010001) (297699680)
...
根据结果可以判断出, TA, TB 线程是并行执行的, TB 线程对象头是无锁状态 **(00000001)**, 而TA 线程调用的普通方法对象头是一个轻量级锁, TB 是轻量级锁的原因并不是因为
线程竞争资源锁膨胀导致的, 是因为偏向锁在 JVM 中有 4s 的开启延迟, 将延迟关闭, 在编译条件中设置偏向锁延迟开启时间为 0：
 `-XX:BiasedLockingStartupDelay=0` 此时在管查打印现象:
...
a 线程获得锁, 对象头信息为: com.anzhi.twoobjecttwolock.HasSelfPrivateNum object internals:
 OFFSET  SIZE   TYPE DESCRIPTION                               VALUE
      0     4        (object header)                           05 30 01 be (00000101 00110000 00000001 10111110) (-1107218427)
      4     4        (object header)                           c6 7f 00 00 (11000110 01111111 00000000 00000000) (32710)
      8     4        (object header)                           43 c1 00 f8 (01000011 11000001 00000000 11111000) (-134168253)
     12     4    int HasSelfPrivateNum.num                     0
Instance size: 16 bytes
Space losses: 0 bytes internal + 0 bytes external = 0 bytes total
...
可以看到 TA TB 线程中的对象头都是轻量级锁


### JVM 底层判断同步方法依据

将` public synchronized void addI`  同步方法, 使用命令 `javap -c -v FileName.class` 转换成字节码指令后，
...
可以看到同步方法块上有 flag标记ACC_SYNCHRONIZED，表明此方法是同步的。非同步方法flags并没有ACC_SYNCHRONIZED标记。JVM 通过检测
ACC_SYNCHRONIZED 同步标志位来执行对应的锁指令
  ...
  public void addIIII(java.lang.String);
    ...
    Code:
      stack=3, locals=6, args_size=2
       ...
       150: return
```
同步代码块有1个monitorenter，但是却有2个monitorexit，这是因为JVM为了保证方法在出异常时也能正常释放锁，编译器会自动产生一个异常处理器，
...
![偏向锁的获得和撤销流程](../图片/6.biasLockGetAndRelease.png)
...
![偏向锁的获得和撤销流程](../图片/7.LightWeightLockBloat.png)
