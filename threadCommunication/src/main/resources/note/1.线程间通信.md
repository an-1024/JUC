...
* [wait/notify 机制](#waitnotify-机制)
    * [wait/notify 机制的原理](#waitnotify-机制的原理)
        * [wait() 方法的基本使用](#wait-方法的基本使用)
* [生产者/消费者模式的实现](#生产者消费者模式的实现)
    * [一生产/一消费：操作值](#一生产一消费操作值)
    * [一生产/一消费： 操作栈](#一生产一消费-操作栈)
    * [一生产/多消费者：操作栈](#一生产多消费者操作栈)
* [通过管道进行线程间通信](#通过管道进行线程间通信)
    * [字节流](#字节流)
    * [字符流](#字符流)
* [join() 方法的使用](#join-方法的使用)
* [ThreadLocal 的使用](#threadlocal-的使用)
    * [ThreadLocal 设计](#threadlocal-设计)
    * [使用场景](#使用场景)
        * [1. 每个线程需要一个独享的对象](#1-每个线程需要一个独享的对象)
        * [2. 当前用户信息需要被线程内所有方法共享](#2-当前用户信息需要被线程内所有方法共享)
* [ThreadLocal 方法说明](#threadlocal-方法说明)
    * [initialValue](#initialvalue)
    * [set](#set)
    * [get](#get)
* [ThreadLocal 原理](#threadlocal-原理)
    * [ThreadLocal Set 方法原理](#threadlocal-set-方法原理)
        * [ThreadLocal 的hashCode](#threadlocal-的hashcode)
        * [确定新值插入哈希表中的位置](#确定新值插入哈希表中的位置)
        * [怎么解决哈希冲突](#怎么解决哈希冲突)
        * [如何解决 "脏" Entry](#如何解决-脏-entry)
    * [ThreadLocal::get方法的原理](#threadlocalget方法的原理)
    * [ThreadLocal::remove()方法原理](#threadlocalremove方法原理)
    * [ThreadLocalMap 的类结构体系如下：](#threadlocalmap-的类结构体系如下)
    * [ThreadLocal 内存泄漏分析](#threadlocal-内存泄漏分析)
* [使用 InheritableThreadLocal 体现值的继承特性](#使用-inheritablethreadlocal-体现值的继承特性)
* [值继承特性在源代码中的执行流程](#值继承特性在源代码中的执行流程)
    * [父线程有最新的值，子线程仍是旧值](#父线程有最新的值子线程仍是旧值)
    * [子线程可以感应对象属性值的变化](#子线程可以感应对象属性值的变化)
    * [重写 childValue() 方法实现对继承的值进行加工](#重写-childvalue-方法实现对继承的值进行加工)
